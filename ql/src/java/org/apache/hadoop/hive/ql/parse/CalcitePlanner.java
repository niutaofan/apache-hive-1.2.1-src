      ...
      if (true) {//runCBO
        ...
        try {
          if (this.conf.getBoolVar(HiveConf.ConfVars.HIVE_CBO_RETPATH_HIVEOP)) {
            ...
            // TODO 1. Gen Optimized AST 此处开始CBO优化
            ASTNode newAST = getOptimizedAST();
//            //TODO 新增一个获取优化后的relNode
//            RelNode relNode = getOptimizedRelNode();
   ...
   * 新加方法，获取优化后的RelNode
   * */
  RelNode getOptimizedRelNode() throws SemanticException {
    ASTNode optiqOptimizedAST = null;
    RelNode optimizedOptiqPlan = null;
    CalcitePlannerAction calcitePlannerAction = new CalcitePlannerAction(prunedPartitions);
  ...
  private class CalcitePlannerAction implements Frameworks.PlannerAction<RelNode> {
    ...
    public RelNode apply(RelOptCluster cluster, RelOptSchema relOptSchema, SchemaPlus rootSchema) {
      ...
      try {
        ...
        System.out.println("##########################Gen Calcite Plan##############################################");
        LOG.warn(RelOptUtil.toString(calciteGenPlan));
        System.out.println(RelOptUtil.toString(calciteGenPlan));
      ...
      System.out.println("##########################Apply Pre Join Order optimizations##############################################");
      LOG.warn(RelOptUtil.toString(calcitePreCboPlan));
      System.out.println(RelOptUtil.toString(calcitePreCboPlan));
      ...
      System.out.println("##########################优化后的执行计划##############################################");
      LOG.warn(RelOptUtil.toString(rootRel));
      ...
      System.out.println("##########################优化后的执行计划2##############################################");
      LOG.warn(RelOptUtil.toString(calciteOptimizedPlan));
      System.out.println(RelOptUtil.toString(calciteOptimizedPlan));
...
//      if (HiveConf.getBoolVar(conf, ConfVars.HIVE_CBO_RETPATH_HIVEOP)) {
    ...
    private RelNode applyPreJoinOrderingTransforms(RelNode basePlan, RelMetadataProvider mdProvider) {
      System.out.println("##########################applyPreJoinOrderingTransforms-0##############################################");
      LOG.warn(RelOptUtil.toString(basePlan));
      ...
      System.out.println("##########################Push Down Semi Joins##############################################");
      LOG.warn(RelOptUtil.toString(basePlan));
      System.out.println(RelOptUtil.toString(basePlan));
...
//      if (conf.getBoolVar(HiveConf.ConfVars.HIVE_CBO_RETPATH_HIVEOP)) {
      ...
      System.out.println("##########################JOIN Add not null filters##############################################");
      LOG.warn(RelOptUtil.toString(basePlan));
      System.out.println(RelOptUtil.toString(basePlan));


      // 3. Constant propagation, common filter extraction, and PPD
      basePlan = hepPlan(basePlan,
              true,
              mdProvider,
...
//              HiveFilter.DEFAULT_FILTER_FACTORY, Aggregate.class)
      ...
      System.out.println("##########################Constant propagation, common filter extraction, and PPD##############################################");
      System.out.println(RelOptUtil.toString(basePlan));
      ...
      System.out.println("##########################Projection Pruning##############################################");
      LOG.warn(RelOptUtil.toString(basePlan));
      System.out.println(RelOptUtil.toString(basePlan));
