...
public class Driver implements CommandProcessor {
   ...
   * @return 0 for ok
   *
   * //TODO SQL->AST(Abstract Syntax Tree)->Task（MapRedTask，FetchTask）->QueryPlan（Task集合）->Job（Yarn）
   */
...
//      ASTNode tree = pd.parse(command, ctx);
      ...
      BaseSemanticAnalyzer sem = SemanticAnalyzerFactory.get(conf, tree);//TODO 开始将ASTNode转成QB
      ...
      if (saHooks != null) {
        ...
        //TODO 从AST Node到Phsical Optimize这几个阶段，都是在SemanticAnalyzer.analyzeInternal()方法中进行的
        //TODO 语义解析、生成逻辑执行计划、优化逻辑执行计划、
      ...
      //TODO compile过程为先由ParseDriver将SQL转换为ASTNode，然后由BaseSemanticAnalyzer对ASTNode进行分析，最后将BaseSemanticAnalyzer传入QueryPlan构造函数来创建QueryPlan
